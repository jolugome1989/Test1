...
Mt=2; % Number Tx antennas
Mr=2; % Number Rx antennas
fc=2.4e9; % Carrier frequency
c0=3e8; % Speed of light
lambda_0=c0/fc; % Wavelength
BW=10e6; % Signal bandwidth
nfreq=2e2; % Number of frequency sample points
f=linspace(-BW/2,BW/2,nfreq); % Vector of frequencies
d_f=diff(f);
delta_freq=d_f(1);
t_max=1./delta_freq;
tbin=linspace(0,t_max,nfreq); % delay vector
dist=c0.*tbin; % Distance vector
dist2=linspace(0,nfreq/BW,nfreq)*c0;
wf = hann(nfreq+1,'Periodic');wf = wf(1:end-1); % Window function for calculating Channel Impulse Response
wf=sqrt(mean(abs(wf).^2))^(-1)*wf; % Normalizing the window funciton to preserve the power level
...
io_del=exp(1j*2*pi*f.'*t_p.'); % Frequency phase vector for each IO (fxNio)
n_iter=1e4;
C=zeros(1,n_iter);
H_norm=C;
