...
P_io=rho_power*(dist_io).^(-alpha); % Calculate Path Loss for each IO using log-distance power law equation
del_pha=exp(-1j*(2*pi/lambda_0).*dist_io);
R=10*lambda_0/(2*pi); % Radius UCA
%[ste_ma,AOA,AOD,ste_tx,ste_rx]=ULA_model(io,bs,ue,Mt,Mr,d_sc,lambda_0); % Uniform Linear Array Model for eah IO (MtxMrxNio)
[ste_ma,AOA,AOD,ste_tx,ste_rx]=UCA_model(io,bs,ue,Mt,Mr,lambda_0,R); % Uniform Circular Array Model
n_iter=1e4;
C=zeros(1,n_iter);
H_norm=C;
